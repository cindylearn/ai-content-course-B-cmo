...
- 🔴 **04 直接把脚本拍成一支正常口播视频就行** —— 用脚本里的 persona 出主播，**先 test 一支跑通**。04 不选人、不造脸。
- 🔴 **主讲人从哪来（在这里 mention，别当硬闸门）：**
  - **先 test / 一次性** → **不用装 05**，04 直接照脚本 persona 生成一个主播（seedance 出）。
  - **想全片同一个人 / 用真人本尊 / 锁定的数字人** → 跟她说：**要先装 + 做 `05 AI数字人`**，拿它锁好的 3 张脸参考图回来再拍（这才需要 05；选人 / 九宫格 / 性别气质全在 05）。
- 🔴 **开工只确认这一件（一题一张选项卡）：说话语言 —— 一定要问，别默认英文：** 🇲🇾 **马来西亚华语／中文（默认）** · 中英混（Rojak）· English · Bahasa Malaysia ·（方言走「其他」）。选的语言 = 台词语言 + 选声语言，全片统一。
- （角色是 **AI 人 / 卡通人** 这种类型，在**脚本 / 立项阶段**就定，不在 04。）
