...
- 🔴🔴 **04 只管「把脚本拍成视频」，不选人、不造脸。** 主角 = 脚本 persona（贴受众）+ **开工前就在 `05 AI数字人` 备好的脸参考图**。脸从哪来、AI 人还是真人、九宫格选脸、性别气质 —— **全是 05 的事，04 一概不碰**。（这条内容用 **AI 数字人 / 真人 / 纯图文（无主播）** 这种角色类型，是在**脚本 / 立项阶段**就定好的，不在 04 现决定。）
- 🔴 **开工只确认（一题一张选项卡）：**
  1. **脸参考图备好了没？** 真人的 3 张，或 05 做好的数字人 3 张。🔴 **没备好 = 你还没做 `05` → 先去做 05，别在 04 现问现做。**（纯图文/无主播的内容 → 不需要脸，跳过这条。）
