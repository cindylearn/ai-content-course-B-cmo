...
| 工具 | 链接 | 干什么 | 钱 |
|---|---|---|---|
| **Higgsfield**（Seedance 2.0）| <https://higgsfield.ai> | **生成**口播镜 + B-roll | 按 credit（480p/fast ≈15–18/镜）|
| ⭐ **ChatCut** | <https://www.chatcut.io> · 编辑器 <https://app.chatcut.io> | **剪辑** —— 字幕/花字/音效/BGM/转场。**中文片唯一可行的 AI 剪辑路** | 有免费额度 |
| **Suno** | <https://suno.com> | 背景乐（ChatCut 内也能直接生成）| 有免费额度 |
| **ffmpeg** | `brew install ffmpeg` | 拼接 / 侦测硬切点 / 探测规格 | 免费开源 |
| **Pexels / Pixabay** | <https://pexels.com> · <https://pixabay.com> | 免费商用素材（ChatCut 内建 `search_stock_media` 直接搜）| 免费商用 |
| **Notion** | <https://notion.so> | 内容矩阵（分镜表 / 排期）| 免费够用 |
| **Google Drive** | — | 存原片。**⚠️ 一定要传原片，不是 `_web` 压缩档**（实测：4.6MB 的 480p 剪出来整支糊，原片 100MB）| — |
...
| **剪映专业版 / CapCut** | <https://www.capcut.cn> | **草稿加密** → AI 程序化剪不了。只能手动或 UI 自动化（慢且脆）。**免费功能其实够用，但 AI 帮不了你** |
